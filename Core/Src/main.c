...
static void Log_WaitBoot(void);
static void Log_CpltBoot(void);
/* USER CODE END PFP */
...
static void Log_WaitBoot(void)
{
	...
	ssd1306_WriteString("BOOT", Font_16x26, White);
...
static void Log_CpltBoot(void)
{
	...
	ssd1306_SetCursor(0, 16);
	ssd1306_WriteString("COMPLETE", Font_11x18, White);
	ssd1306_UpdateScreen(&hi2c2);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : B1_Pin BTN2_Pin */
  GPIO_InitStruct.Pin = B1_Pin|BTN2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
